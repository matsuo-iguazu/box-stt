...
1. IBM Cloud コンソールで Watson STT インスタンスを開きます。
...
2. 親フォルダの中に、以下の 3 つの子フォルダを作成します。
   ...
   * `text`（テキスト出力用）
   * `done`（処理済み移動用）
3. 各フォルダを開き、ブラウザの URL 末尾にある数字（フォルダ ID）をコピーして、`.env` の `BOX_SPEECH_FOLDER_ID`, `BOX_TEXT_FOLDER_ID`, `BOX_DONE_FOLDER_ID` にそれぞれ貼り付けます。

### STEP 4: Box Platform アプリケーションの定義

1. **アプリ作成**: [Box開発者コンソール](https://developer.box.com/)で「アプリの新規作成」>「カスタムアプリ」を選択。
   * **認証方法**: 「クライアント資格情報許可 (Client Credentials Grant)」を選択。
2. **基本情報取得**: 「構成」タブで以下を確認します。
   * **クライアント ID**: コピーし、`.env` の `BOX_CLIENT_ID` へ貼り付けます。
   * **クライアントシークレット**: コピーし、`.env` の `BOX_CLIENT_SECRET` へ貼り付けます。
   * **Enterprise ID**: 画面下部またはアカウント設定から確認し、`.env` の `BOX_ENTERPRISE_ID` へ貼り付けます。
3. **アプリケーションスコープ**: 以下にチェックを入れます。
   * `Boxに格納されているファイルとフォルダの書き込み`
   * `Webhookの管理`
4. **親フォルダへの招待**:
   * アプリの「構成」タブにある **「サービスアカウント ID」**（自動生成されたメールアドレス形式のもの）をコピーします。
   * Box ユーザー画面に戻り、STEP 3 で作成した **親フォルダ** に、このサービスアカウントを「編集者」として招待します。
5. **アプリの承認**:
   * Box 管理コンソール > アプリ > カスタムアプリ管理 を開き、本アプリを承認します。
